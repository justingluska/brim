#!/bin/zsh
# Xcode Cloud post-clone hook for Brim.
#
# Runs on Apple's build machine after the clone and before Xcode Cloud opens
# Brim.xcodeproj. The project file is gitignored and generated from project.yml,
# so it is produced here. This directory has to sit next to the .xcodeproj.
set -euo pipefail
...
# marketing version is stamped here. Xcode Cloud clones shallow and without
# tags, so fetch history + tags first; with no tag the project.yml value stays.
plist_set() {
  /usr/libexec/PlistBuddy -c "Set :$1 $2" Brim/Info.plist 2>/dev/null \
    || /usr/libexec/PlistBuddy -c "Add :$1 string $2" Brim/Info.plist
}
...
if [[ -n "$TAG" ]]; then
  plist_set CFBundleShortVersionString "${TAG#v}"
